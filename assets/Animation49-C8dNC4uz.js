import{a as e}from"./lil-gui.esm-jlbWO7FJ.js";import{Dr as t,Dt as n,Gt as r,P as i,Vt as a,Xn as o,Zn as s,c,g as l,l as u,r as d,tt as f,u as p,vt as m,zt as h}from"./three.module-TVF63cYk.js";import{n as g,r as _,t as v}from"./OutputPass-CxDAHfy4.js";import{t as y}from"./UnrealBloomPass-C17ZlHgr.js";import{i as b,n as x,r as S,t as C}from"./GUIHelper-DspWBXk2.js";import{t as w}from"./OrbitControls-Ju8LL-qO.js";var T=(e=!1)=>({blending:5,blendEquation:100,blendSrc:e?201:204,blendDst:201,blendEquationAlpha:100,blendSrcAlpha:e?200:201,blendDstAlpha:e?201:205,premultipliedAlpha:e}),E=e=>Object.assign(e.blendMaterial,T(!0)),D=class{constructor(e,t={}){this.canvas=e;let n={bloom强度:.1,bloom半径:.1,bloom阈值:.8,bloom音频增幅:.1,echo残影消退:.25,echo叠加透明度:.45,跳动幅度:.1,跳动音频敏感度:.1,流动音频敏感度:.5,扩张音频敏感度:.12,抖动音频敏感度:.5,大小音频敏感度:.8,鼓点爆发强度:.5,柱数量:128,柱宽度:.2,柱高度系数:2,柱最小高度:.2,柱颜色速度:.02,鼓点闪光衰减:.92,粒子透明度:.3,颜色饱和度:1.5,颜色冷暖敏感度:.6,粒子分布半径:1,粒子数量:5e3,中心区域亮度:0,粒子大小:5,cameraPosition:{x:0,y:0,z:12},beatSensitivity:.1};this.settings={...n,...t},this.defaultSettings=n,this.scene=null,this.barScene=null,this.camera=null,this.renderer=null,this.controls=null,this.composer=null,this.bloomPass=null,this.particles=null,this.particleMat=null,this.barMesh=null,this.barGeo=null,this.barMat=null,this.barDummy=new h,this.barColor=new l,this.echoCanvas=null,this.echoCtx=null,this.echoTempCanvas=null,this.echoTempCtx=null,this.audioFeature={bass:0,mid:0,high:0,energy:0,motion:0},this.brightness=0,this.kick=0,this.hasAudioData=!1,this._beatDecay=0,this._isReady=!1,this._time=0,this.fftData=new Uint8Array(256),this.beatFlash=0,this.gui=null,this.guiContainer=null,this.guiVisible=!1,this._lastFrameTime=-1,this.time=0,this.settingsButton=null,this.initPromise=this.init(),this.initPromise.then(()=>{this._isReady=!0}).catch(e=>{console.error(`❌ Animation49 初始化失败:`,e)})}async init(){try{return await this.setupThreeJS(),this.setupEchoCanvas(),this.createParticles(this.settings.粒子数量),this.createBars(this.settings.柱数量),this.setupPostProcessing(),this.setupGUI(),this.setupSettingsButton(),this.resizeHandler=()=>this.onWindowResize(),window.addEventListener(`resize`,this.resizeHandler),console.log(`✅ Animation49 初始化成功`),!0}catch(e){throw console.error(`❌ Animation49 初始化失败:`,e),e}}async setupThreeJS(){this.scene=new o,this.barScene=new o,this.camera=new a(60,window.innerWidth/window.innerHeight,.1,1e3),this.camera.position.set(this.settings.cameraPosition.x,this.settings.cameraPosition.y,this.settings.cameraPosition.z),this.camera.lookAt(0,0,0),this.renderer=new d({canvas:this.canvas,antialias:!0,alpha:!0,powerPreference:`high-performance`}),this.renderer.setSize(window.innerWidth,window.innerHeight),this.renderer.setClearColor(0,0),this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,2)),this.renderer.outputColorSpace=m,this.canvas.style.position=`fixed`,this.canvas.style.left=`0`,this.canvas.style.top=`0`,this.canvas.style.zIndex=`0`,this.canvas.style.pointerEvents=`none`,this.canvas.style.opacity=`0`,this.canvas.style.width=window.innerWidth+`px`,this.canvas.style.height=window.innerHeight+`px`;let e=document.createElement(`div`);e.style.cssText=`position:fixed;top:0;left:0;width:100%;height:100%;z-index:5;cursor:default!important`,e.id=`Animation49-controls-overlay`,document.body.appendChild(e),this.controlsOverlay=e,this.controls=new w(this.camera,e),this.controls.enableDamping=!0,this.controls.dampingFactor=.08,this.controls.minDistance=3,this.controls.maxDistance=1/0,this.controls.saveState(),this.controls.update()}setupEchoCanvas(){this.echoCanvas=document.createElement(`canvas`),this.echoCanvas.id=`Animation49-echo-canvas`,this.echoCtx=this.echoCanvas.getContext(`2d`),this.echoCanvas.width=window.innerWidth,this.echoCanvas.height=window.innerHeight,this.echoCanvas.style.position=`fixed`,this.echoCanvas.style.left=`0`,this.echoCanvas.style.top=`0`,this.echoCanvas.style.width=`100%`,this.echoCanvas.style.height=`100%`,this.echoCanvas.style.zIndex=`1`,this.echoCanvas.style.pointerEvents=`none`,this.echoCanvas.style.background=`transparent`,document.body.appendChild(this.echoCanvas),this.visibleCanvas=this.echoCanvas,this.echoTempCanvas=document.createElement(`canvas`),this.echoTempCtx=this.echoTempCanvas.getContext(`2d`),this.echoTempCanvas.width=this.echoCanvas.width,this.echoTempCanvas.height=this.echoCanvas.height}createBars(e){this.barMesh&&(this.barMesh.geometry.dispose(),this.barMesh.material.dispose(),this.barScene.remove(this.barMesh)),this.barGeo=new c(this.settings.柱宽度,1,this.settings.柱宽度),this.barMat=new n,this.barMesh=new f(this.barGeo,this.barMat,e),this.barMesh.instanceMatrix.setUsage(i);for(let t=0;t<e;t++){let n=t/e*Math.PI*2,r=3.5;this.barDummy.position.set(Math.cos(n)*r,0,Math.sin(n)*r),this.barDummy.rotation.y=-n,this.barDummy.scale.set(1,1,1),this.barDummy.updateMatrix(),this.barMesh.setMatrixAt(t,this.barDummy.matrix),this.barMesh.setColorAt(t,new l().setHSL(t/e,1,.6))}this.barMesh.instanceColor.needsUpdate=!0,this.barMesh.instanceMatrix.needsUpdate=!0,this.barScene.add(this.barMesh)}createParticleGeometry(e){let t=new Float32Array(e*3),n=new Float32Array(e*3),r=new Float32Array(e);for(let i=0;i<e;i++){let e=5+Math.random()*8,a=Math.random()*Math.PI*2,o=(Math.random()-.5)*2;t[i*3]=Math.cos(a)*e,t[i*3+1]=o,t[i*3+2]=Math.sin(a)*e;let s=Math.random(),c=new l().setHSL(s,1,.65);n[i*3]=c.r,n[i*3+1]=c.g,n[i*3+2]=c.b,r[i]=1+Math.random()*2}let i=new p;return i.setAttribute(`position`,new u(t,3)),i.setAttribute(`color`,new u(n,3)),i.setAttribute(`size`,new u(r,1)),i}createParticles(e){this.particles&&(this.scene.remove(this.particles),this.particles.geometry.dispose()),this.particleMat||=new s({transparent:!0,depthWrite:!1,...T(),uniforms:{uTime:{value:0},uBass:{value:0},uMid:{value:0},uHigh:{value:0},uEnergy:{value:0},uMotion:{value:0},uBrightness:{value:0},uBeat:{value:0},uColorReaction:{value:this.settings.颜色冷暖敏感度},uOpacity:{value:this.settings.粒子透明度},uColorIntensity:{value:this.settings.颜色饱和度},uRadiusScale:{value:this.settings.粒子分布半径},uCenterBrightness:{value:this.settings.中心区域亮度},uSizeScale:{value:this.settings.粒子大小},uBounceAmplitude:{value:this.settings.跳动幅度},uMidBounce:{value:this.settings.跳动音频敏感度},uFlowSensitivity:{value:this.settings.流动音频敏感度},uExpandSensitivity:{value:this.settings.扩张音频敏感度},uJitterSensitivity:{value:this.settings.抖动音频敏感度},uSizeSensitivity:{value:this.settings.大小音频敏感度},uBurstSensitivity:{value:this.settings.鼓点爆发强度}},vertexShader:`
